...
const displayPokemanPopup = (pokeman, evolution) => {
        ...
        fetch(url).then((res) => res.json()).then((gen8pokemon) => {
            ...
            fetch(urlt).then((res) => res.json()).then((type_chart) => {
                    ...
                    fetch(url7).then((res) => res.json()).then((desc) => {
                                ...
                                const types = `Takes ${type_chart[pokeman.id-1].normal} times damage from normal. <br> Takes ${type_chart[pokeman.id-1].fire} times damage from fire. <br> Takes ${type_chart[pokeman.id-1].water} times damage from water. <br> Takes ${type_chart[pokeman.id-1].electric} times damage from electric. <br> Takes ${type_chart[pokeman.id-1].grass} times damage from grass. <br> Takes ${type_chart[pokeman.id-1].ice} times damage from ice. <br> Takes ${type_chart[pokeman.id-1].fighting} times damage from fighting. <br> Takes ${type_chart[pokeman.id-1].poison} times damage from poison. <br> Takes ${type_chart[pokeman.id-1].ground} times damage from ground. <br> Takes ${type_chart[pokeman.id-1].flying} times damage from flying. <br> Takes ${type_chart[pokeman.id-1].psychic} times damage from psychic. <br> Takes ${type_chart[pokeman.id-1].bug} times damage from bug. <br> Takes ${type_chart[pokeman.id-1].rock} times damage from rock. <br> Takes ${type_chart[pokeman.id-1].ghost} times damage from ghost. <br> Takes ${type_chart[pokeman.id-1].dragon} times damage from dragon. <br> Takes ${type_chart[pokeman.id-1].dark} times damage from dark. <br> Takes ${type_chart[pokeman.id-1].steel} times damage from steel. <br> Takes ${type_chart[pokeman.id-1].fairy} times damage from fairy.`
        ...
        const htmlString = `
            ...
            pokeman.weight
        }<br> Abilities: ${ability}<br> Base Stats: <br>${stats}<br>${species}${description}<br>${evolchain}<br>${types}<br>${movers}</p>
                </div>
            </div>
        `;
